...
module.exports = {
  ...
  run: async (message, args, client) => {
    ...
    if (target.roles.highest.position >= message.guild.me.roles.highest.position) return message.reply(`User has a higher role then me.`)
        let muted = await moderations.findOne({user: target.id, action: "Mute", active: true, guild: message.guild.id})
        if (muted) return message.reply(`User already is muted.`)
    ...
    target.send({ content: `You have been muted in ${message.guild.name} for ${reason}` }).catch(console.log)
...
let muterole = message.guild.roles.cache.filter((roless) => roless.name == "Muted").first()
    ...
    message.reply({ content: `**${target.user.tag}** has been muted.` })
  }
}