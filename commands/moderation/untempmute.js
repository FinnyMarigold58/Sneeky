...
module.exports = {
  name: "untempmute",
  description: "Untempmute a muted guild member",
  usage: `${process.env.PREFIX}untempmute [user]`,
  ...
  run: async (message, args, client) => {
    ...
    if (target.roles.highest.position >= message.member.roles.highest.position || target.id === message.guild.ownerId) return message.reply({ content: "You can not untempmute this person." })
    if (target.roles.highest.position >= message.guild.me.roles.highest.position) return message.reply(`User has a higher role then me.`)
        let muted = await moderations.findOne({user: target.id, action: "tempmute", active: true})
        if (!muted) return message.reply(`User is not tempmuted.`)
    ...
    target.send({ content: `You have been untempmuted in ${message.guild.name}` }).catch(console.log)
...
let muterole = message.guild.roles.cache.filter((roless) => roless.name == "Muted").first()
    ...
    message.reply({ content: `**${target.user.tag}** has been untempmuted.` })
  }
}